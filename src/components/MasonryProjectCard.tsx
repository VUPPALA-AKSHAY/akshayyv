...
export const MasonryProjectCard = ({ project, className = "" }: MasonryProjectCardProps) => {
  ...
  return (
    ...
    >
      <div
        className={`flex flex-col gap-3 w-full p-1 bg-white dark:bg-white/10 border border-black/10 dark:border-white/5 rounded-[10px] transition-all duration-300 ease-out group-has-hover:opacity-40 group-has-hover:group-hover/item:opacity-100 group-has-hover:group-hover/item:border-[var(--brand)] group-has-hover:group-hover/item:dark:border-[var(--brand)] group-has-hover:group-hover/item:scale-[1.02] group-has-hover:group-hover/item:shadow-lg group-has-hover:group-hover/item:shadow-black/5 dark:group-has-hover:group-hover/item:shadow-black/20 ${className}`}
      >
        ...
        <div className="w-full px-2 pb-4 flex items-center gap-2">
          {project.isNew && (
            <span className="px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-[var(--on-brand)] rounded-full" style={{ background: 'var(--brand)' }}>
              New
            </span>
          )}
          <span className="text-[15px] leading-7 text-black/80 group-has-hover:hover:text-[var(--brand)] dark:text-white/80 dark:group-has-hover:hover:text-[var(--brand)] font-medium transition-colors duration-300">
            {project.title}
          </span>
        </div>
      </div>
